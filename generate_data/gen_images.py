...
parser.add_argument(
    "--num_samples", type=int, default=1024, help="Total number of samples to generate"
)
parser.add_argument(
    "--latent_dim",
    type=int,
...
parser.add_argument(
    "--batch_size", type=int, default=128, help="Batch size for generating images"
)
parser.add_argument(
    "--class_name",
    type=str,
...
parser.add_argument("--class_id", type=int, default=0, help="ID of class {0,...,999}")
parser.add_argument(
    "--truncation",
    type=float,
    default=1,
...
parser.add_argument(
    "--save_dir", type=str, default="samples/test", help="Save directory for files"
)
parser.add_argument("--gpu_id", type=str, default="0", help="GPU ID(s) to use")
parser.add_argument(
    "--add",
    action="store_true",
    default=False,
    help="Adds more data to existing directory" "Overides directory check",
)
...
def main(args):
    ...
    if args.add:
        ...
        start_batch_num = max(
            [int(x.split("/")[-1].replace(".pt", "")) for x in existing_batches]
        )
    ...
    idx = 1
    for b in tqdm(
        range(start_batch_num, num_batches),
        desc="Batch: {}/{}".format(idx, num_batches),
    ):
        idx += 1
        ...
        # Prepare inputs
        if args.class_id:
            class_vector = np.zeros(
                (args.batch_size, NUM_IMAGENET_CLS), dtype=np.float32
            )
            class_vector[:, args.class_id] = 1
        elif args.class_name:
            class_vector = one_hot_from_names(
                [args.class_name], batch_size=args.batch_size
            )
        else:
